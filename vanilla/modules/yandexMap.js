...
export function initYandexMap() {
  ...
  const initMap = () => {
    ...
    ymaps.ready(() => {
      const mapContainer = qs("#yandex-map");
      ...
      function renderList() {
        ...
        const filteredPoints = MAP_POINTS.filter(point => 
          state.filter === "all" || point.type === state.filter
        );
        ...
        listBody.innerHTML = filteredPoints.map(point => `
          <button
      ...
      if (filterRow) {
        filterRow.innerHTML = MAP_TYPES.map(type => `
          <button
        ...
        filterRow.querySelectorAll('button[data-filter]').forEach(btn => {
          btn.addEventListener('click', () => {
            ...
            filterRow.querySelectorAll('button[data-filter]').forEach(b => {
              ...
              if (span) {
                span.style.background = isActive ? 'rgba(255,255,255,0.25)' : 'rgba(184,230,213,0.4)';
